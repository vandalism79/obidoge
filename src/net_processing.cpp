...
// Internal stuff
namespace {
    // ObiDoge debug: last P2P message command seen on this thread (for banscore attribution)
    static thread_local std::string g_obd_last_netmsg_cmd;
...
void Misbehaving(NodeId pnode, int howmuch)
{
    ...
    // ObiDoge debug: attribute banscore increments to the last processed P2P message command on this thread
    const std::string obd_cmd = g_obd_last_netmsg_cmd.empty() ? "<unknown>" : g_obd_last_netmsg_cmd;

    if (state->nMisbehavior >= banscore && state->nMisbehavior - howmuch < banscore)
    {
        LogPrintf("%s: %s peer=%d (%d -> %d) BAN THRESHOLD EXCEEDED cmd=%s\\n",
                  __func__, state->name, pnode, state->nMisbehavior - howmuch, state->nMisbehavior, obd_cmd);
        state->fShouldBan = true;
    }
    else
    {
        LogPrintf("%s: %s peer=%d (%d -> %d) cmd=%s\\n",
                  __func__, state->name, pnode, state->nMisbehavior - howmuch, state->nMisbehavior, obd_cmd);
    }
}
...
// ObiDoge: Rialto: Relay a Rialto message by adding to Rialto inventory for all peers apart from origin
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    // ObiDoge debug: record current message command for Misbehaving() logging
    g_obd_last_netmsg_cmd = strCommand;
    ...
    else if (strCommand == NetMsgType::INV)
    {
        ...
        for (CInv &inv : vInv)
        {
            ...
            else
            {
                ...
                // ObiDoge: Rialto inventory should never interfere with normal tx relay.
                // Only apply Rialto checks to MSG_RIALTO inventory.
                if (inv.type == MSG_RIALTO) {
                    // If we don't support Rialto, just ignore the inventory.
                    if ((connman->GetLocalServices() & NODE_RIALTO) != NODE_RIALTO) {
                        LogPrint(BCLog::NET, "rialto inv (%s) ignored (we do not relay rialto). peer=%d\n", inv.hash.ToString(), pfrom->GetId());
                    } else if ((pfrom->nServices & NODE_RIALTO) != NODE_RIALTO) {
                        LogPrint(BCLog::NET, "rialto inv (%s) from non-rialto peer. peer=%d\n", inv.hash.ToString(), pfrom->GetId());
                        Misbehaving(pfrom->GetId(), 20);
...
bool PeerLogicValidation::SendMessages(CNode* pto, std::atomic<bool>& interruptMsgProc)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        std::vector<CInv> vInv;
        {
            ...
            // ObiDoge: Rialto: Add messages (Note: We already gated on their NODE_RIALTO support when adding to the inventory)
            for (const uint256& hash : pto->rialtoInventoryToSend)
                vInv.push_back(CInv(MSG_RIALTO, hash));
            pto->rialtoInventoryToSend.clear();

            // Expire old relay messages
            // ObiDoge: Rialto: TODO: Maybe not here though? Also figure out why joining clients don't get these (we never add to their inventories I guess)
            /*
