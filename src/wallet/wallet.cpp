...
#include <rialto.h>  // ObiDoge: Rialto: For IsValidNick()
...
#include <script/ismine.h>  // ObiDoge: Hive
...
bool CWallet::SignTransaction(CMutableTransaction &tx)
{
    ...
    for (const auto& input : tx.vin) {
        ...
        if (!ProduceSignature(TransactionSignatureCreator(this, &txNewConst, nIn, amount, SIGHASH_ALL | SIGHASH_FORKID), scriptPubKey, sigdata)) {	// ObiDoge: Replay attack protection
            return false;
        }
...
// ObiDoge: Hive: Unlock for hive mining purposes only.
// ObiDoge: Rialto: Renamed from fWalletUnlockHiveOnly to reflect usage.
    ...
    unsigned int nBytes;
    {
        ...
        if (sign)
        {
            ...
            for (const auto& coin : setCoins)
            {
                ...
                if (!ProduceSignature(TransactionSignatureCreator(this, &txNewConst, nIn, coin.txout.nValue, SIGHASH_ALL | SIGHASH_FORKID), scriptPubKey, sigdata))     // ObiDoge: Include fork ID in the signature
                {
                    strFailReason = _("Signing transaction failed");
                    return false;
                } else {
