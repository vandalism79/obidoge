...
#include <warnings.h>
#include <rialto.h>         // ObiDoge: Rialto
#include <script/ismine.h>  // ObiDoge: Rialto
#include <rpc/server.h>     // ObiDoge: Rialto
#include <wallet/wallet.h>  // ObiDoge: Rialto
#include <base58.h>         // ObiDoge: Rialto: for DecodeDestination()
...
#include <miner.h>  // ObiDoge: Hive
#include <merkleblock.h> // ObiDoge: Hive for merkle transaction check in block

#if defined(NDEBUG)
# error "ObiDoge cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Litecoin Signed Message:\n";   // ObiDoge: Should still use LTC's strMessageMagic so that pre-fork sigs validate
...
std::unique_ptr<CRialtoWhitePagesDB> pwhitepages;     // ObiDoge: Rialto: Global white pages
std::unique_ptr<CRialtoWhitePagesDB> pmynicks;        // ObiDoge: Rialto: Our own nicks
std::unique_ptr<CRialtoWhitePagesDB> pblockednicks;   // ObiDoge: Rialto: Blocked nicks we'll ignore messages from
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s OBD additional fees, %d delta bytes\n",
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
    ...
    // ObiDoge: skip PoW check for the genesis block; it's anchored by params.
    if (block.GetHash() == consensusParams.hashGenesisBlock)
        return true;

    // ObiDoge: Hive is permanently disabled. Even if the old IsHiveMined()
    // heuristic would have treated this as Hive, we now treat every block
    // as standard PoW and only verify the MinotaurX PoW hash.
...
{
    // ObiDoge: clean, halving-aware subsidy with logging.
    // - Base block reward: 475 OBD
    // - Halving every consensusParams.nSubsidyHalvingInterval blocks
    // - No premine.

    static const CAmount INITIAL_SUBSIDY = 475 * COIN;

    // Genesis: no subsidy
    if (nHeight == 0) {
        LogPrintf("ObiDoge GetBlockSubsidy: height=%d -> subsidy=0 (genesis)\n", nHeight);
        return 0;
    }

    // Defensive: avoid division by zero or garbage halving intervals.
    int interval = consensusParams.nSubsidyHalvingInterval;

    if (interval <= 0) {
        // This should NEVER happen on a correctly configured chainparams.
        LogPrintf("ObiDoge GetBlockSubsidy: height=%d INVALID interval=%d, forcing 1051200\n",
                  nHeight, interval);
        interval = 1051200; // Failsafe so daemon doesn't crash
    }

    int halvings = nHeight / interval;

    if (halvings >= 64) {
        LogPrintf("ObiDoge GetBlockSubsidy: height=%d -> subsidy=0 (halvings=%d)\n",
                  nHeight, halvings);
        return 0;
    }

    CAmount nSubsidy = INITIAL_SUBSIDY;
    nSubsidy >>= halvings; // Apply halving schedule

    LogPrintf("ObiDoge GetBlockSubsidy: height=%d interval=%d halvings=%d subsidy=%lld\n",
              nHeight,
              interval,
              halvings,
              (long long)nSubsidy);
...
// ObiDoge: Hive: Return the current cost for a single worker bee
CAmount GetBeeCost(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    // ObiDoge: MinotaurX+Hive1.2: Note that this doesn't change; bee cost remains calculated against the base subsidy. Great!
...
int32_t ComputeBlockVersion(const CBlockIndex* pindexPrev, const Consensus::Params& params)
{
    ...
    // ObiDoge: MinotaurX+Hive1.2: Set bit 29 to 0
...
public:
    ...
    {
        // ObiDoge: MinotaurX+Hive1.2: Versionbits always active since powforktime and high bits repurposed at minotaurx UASF activation;
...
static unsigned int GetBlockScriptFlags(const CBlockIndex* pindex, const Consensus::Params& consensusparams) {
    ...
    // ObiDoge: Enforce use of correct fork ID
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // ObiDoge: MinotaurX-only chain. No Hive and no reward splitting.
    // Use the full block subsidy for all mined blocks, plus transaction fees.
    ...
    if (IsMinotaurXEnabled(pindex->pprev, chainparams.GetConsensus())) {
        // MinotaurX is active: keep full reward; no Hive multiplier or halving here.
        // (All reward sharing has been removed from ObiDoge.)
    }
    ...
    // ObiDoge: No Scrypt era or premine enforcement.
    // The original ObiDoge rule required that block (lastScryptBlock + 1)
    // pay its coinbase output to a fixed premine address (premineOutputScript).
    // ObiDoge is a pure MinotaurX fair-launch chain with no premine, so we
...
void static UpdateTip(const CBlockIndex *pindexNew, const CChainParams& chainParams) {
    ...
    if (!IsInitialBlockDownload())
    {
        ...
        for (int i = 0; i < 100 && pindex != nullptr; i++)
        {
            ...
            // ObiDoge: MinotaurX+Hive1.2: Mask out blocktype before checking for possible unknown upgrade
            if (IsMinotaurXEnabled(pindex, chainParams.GetConsensus())) {
                // ObiDoge: Rialto: Added explicit cast to prevent compile warning
                if ((pindex->nVersion & (int32_t)0xFF00FFFF) != nExpectedVersion && !pindex->GetBlockHeader().IsHiveMined(chainParams.GetConsensus()))
                    ++nUpgraded;
            } else {
                // ObiDoge: Hive: Don't warn about unexpected version in Hivemined blocks
...
bool CChainState::ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    // ObiDoge: Rialto: Check for nick creation txs
...
{
    // ObiDoge: MinotaurX-only PoW with a special case for the hardcoded genesis block.
    // The original ObiDoge regtest genesis is created under MinotaurX-only PoW rules, so
    // we do NOT enforce MinotaurX PoW on the genesis header. All other blocks must
    // pass MinotaurX-based CheckProofOfWork().

    if (fCheckPOW) {
        // Skip PoW check for the genesis block only.
        if (block.GetHash() != consensusParams.hashGenesisBlock) {
            if (!CheckProofOfWork(block.GetPoWHash(), block.nBits, consensusParams)) {
    ...
    // Keep the usual timestamp check (adapt this if your file had extra checks here)
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // ObiDoge: Hive is disabled at consensus level, but we do NOT hard-reject
    // blocks here based on legacy Hive detection, because false positives
    // could prevent valid MinotaurX PoW blocks from being mined. Any real
    // consensus invalidity will be caught by the standard PoW and header checks
    // that follow.
//
//    if (block.IsHiveMined(consensusParams)) {
//        return state.DoS(100, false, REJECT_INVALID, "hive-disabled", false, "hive is disabled on ObiDoge");
//    }
//
    ...
    // Check transactions
    for (const auto& tx : block.vtx)
        if (!CheckTransaction(*tx, state, true))    // ObiDoge: Fix CVE-2018-17144
...
// ObiDoge: Hive: Check if Hive is activated at given point
bool IsHiveEnabled(const CBlockIndex* pindexPrev, const Consensus::Params& params)
{
    // ObiDoge: Hive is permanently disabled on all networks.
    // We keep the function so existing calls compile, but it always returns false.
    return false;
}

// ObiDoge: Hive: Check if Hive 1.1 is activated at given point
bool IsHive11Enabled(const CBlockIndex* pindexPrev, const Consensus::Params& params)
{
    // ObiDoge: Hive 1.1 is permanently disabled.
    return false;
}

// ObiDoge: MinotaurX+Hive1.2: Check if MinotaurX is activated at given point
bool IsMinotaurXEnabled(const CBlockIndex* pindexPrev, const Consensus::Params& params)
{
    // ObiDoge: MinotaurX is always considered active for PoW.
    // (We will later enforce MinotaurX-only hashing at GetPoWHash and validation.)
    return true;
}

// ObiDoge: Rialto: Check if Rialto is activated at given point
bool IsRialtoEnabled(const CBlockIndex* pindexPrev, const Consensus::Params& params) {
    // ObiDoge: Rialto messaging is permanently disabled at consensus level.
    // Even if wallet/UI/RPC calls this helper, it will never report Rialto as active.
    LOCK(cs_main);
    return false;
}

// ObiDoge: Rialto: Check if a nick is already registered (helper to provide access to White Pages DB)
bool RialtoNickExists(const std::string nick) {
    LOCK(cs_main);
    return pwhitepages->NickExists(nick);
}

// ObiDoge: Rialto: Check if a nick is ours
bool RialtoNickIsLocal(const std::string nick) {
    LOCK(cs_main);
    return pmynicks->NickExists(nick);
}

// ObiDoge: Rialto: Grab rialto pubkey for given nick, using global White Pages
...
/*
// ObiDoge: Rialto: Grab rialto pubkey for given nick, using local White Pages
...
// ObiDoge: Rialto: Grab rialto privkey for given nick, using local White Pages and wallet.
...
// ObiDoge: Hive: Get the well-rooted deterministic random string (see whitepaper section 4.1)
...
// ObiDoge: Hive: Get tx by given hash, from a block at given chain height
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // ObiDoge: MinotaurX-only PoW. Hive is permanently disabled at consensus
    // level, so we never perform any Hive-specific contextual checks here.
    const Consensus::Params& consensusParams = params.GetConsensus();

    // Difficulty / PoW contextual checks.
    if (IsMinotaurXEnabled(pindexPrev, consensusParams)) {
        ...
        // Pre-MinotaurX behaviour (should effectively never be used on ObiDoge's
        // live chain, but keep for completeness and early-chain safety).
    ...
    // Check timestamp against previous block (Median Time Past)
    if (pindexPrev != nullptr && block.GetBlockTime() <= pindexPrev->GetMedianTimePast()) {
        return state.Invalid(false, REJECT_INVALID, "time-too-old",
                             "block's timestamp is too early");
    }

    // Future timestamp limit (different window once MinotaurX is active)
    int64_t max_future_block_time = IsMinotaurXEnabled(pindexPrev, consensusParams)
                                      ? MAX_FUTURE_BLOCK_TIME_MINOTAURX
    ...
    if (!IsMinotaurXEnabled(pindexPrev, consensusParams)) {
        // Reject outdated version blocks when 95% (75% on testnet) of the
        // network has upgraded: check for version 2, 3 and 4 upgrades.
        ...
        if (block.nVersion < VERSIONBITS_TOP_BITS &&
            IsWitnessEnabled(pindexPrev, consensusParams)) {
            return state.Invalid(false, REJECT_OBSOLETE,
                                 strprintf("bad-version(0x%08x)", block.nVersion),
                                 strprintf("rejected nVersion=0x%08x block", block.nVersion));
        }
    } else {
        // Once MinotaurX is active, enforce new-style version semantics.
        // Top 8 bits must be zero.
        if (block.nVersion & 0xFF000000) {
            return state.Invalid(false, REJECT_OBSOLETE,
                                 strprintf("bad-version(0x%08x)", block.nVersion),
                                 strprintf("rejected nVersion=0x%08x block (old versionbits)",
                                           block.nVersion));
        }
        ...
        if (blockType >= NUM_BLOCK_TYPES) {
            return state.Invalid(false, REJECT_INVALID, "bad-blocktype",
...
{
    // For the genesis block (prev hash is null), pindexPrev is expected to be null.
    // During reindex/import, we can also encounter blocks whose prev is not yet known; those should be queued/handled
    // by the unknown-parent logic, not contextually checked against a null prev.
    const bool fIsGenesis = block.hashPrevBlock.IsNull();

    if (pindexPrev == nullptr && !fIsGenesis) {
        return state.Invalid(false, REJECT_INVALID, "prev-blk-not-found");
    }

    const int nHeight = (pindexPrev == nullptr) ? 0 : (pindexPrev->nHeight + 1);

    // Start enforcing BIP113 (Median Time Past) using versionbits logic.
    // Always use a safe cutoff when pindexPrev is null (genesis), to avoid null derefs.
    int nLockTimeFlags = 0;
    if (pindexPrev != nullptr) {
        nLockTimeFlags = VersionBitsState(pindexPrev, consensusParams, Consensus::DEPLOYMENT_CSV, versionbitscache) == ThresholdState::THRESHOLD_ACTIVE
    ...
    if (!fIsGenesis && nHeight >= consensusParams.BIP34Height)
    {
