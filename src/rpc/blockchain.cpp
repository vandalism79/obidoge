...
#include <primitives/block.h>       // ObiDoge: MinotaurX+Hive1.2: for POW_TYPE
 ...
 */
// ObiDoge: Hive: Optional getHiveDifficulty param
// ObiDoge: MinotaurX+Hive1.2: Add powType param
double GetDifficulty(const CChain& chain, const CBlockIndex* blockindex, bool getHiveDifficulty = false, POW_TYPE powType = POW_TYPE_SHA256)
{
    if (blockindex == nullptr)
    {
        ...
        // ObiDoge: On genesis (or no previous block), just treat difficulty as 1.0
...
const Consensus::Params& consensusParams = Params().GetConsensus();
    ...
    // ObiDoge: Hive: If tip is PoW and we want hivemined, step back until we find a Hive block
    // ObiDoge: Hive 1.1: Allow there to be multiple hive blocks in the way
    if (getHiveDifficulty) {
        ...
        // ObiDoge: MinotaurX+Hive1.2: Skip over incorrect powTypes
...
// ObiDoge: Hive: Pass through optional getHiveDifficulty param
// ObiDoge: MinotaurX+Hive1.2: Add additional POW_TYPE arg
...
UniValue blockheaderToJSON(const CBlockIndex* blockindex)
{
    ...
    int confirmations = -1;
    auto consensusParams = Params().GetConsensus();     // ObiDoge: MinotaurX-only

    // Only report confirmations if the block is on the main chain
    if (chainActive.Contains(blockindex))
        confirmations = chainActive.Height() - blockindex->nHeight + 1;

    // ObiDoge: MinotaurX-only chain, no Hive.
    result.push_back(Pair("type", "minotaurx"));
    result.push_back(Pair("powtype", "minotaurx"));
    ...
    // ObiDoge: Difficulty is MinotaurX PoW difficulty.
    result.push_back(Pair("difficulty", GetDifficulty(blockindex, false, POW_TYPE_MINOTAURX)));
...
// ObiDoge: MinotaurX+Hive1.2: Allow additional powalgo arg
UniValue getdifficulty(const JSONRPCRequest& request)
{
    ...
    if (!IsMinotaurXEnabled(chainActive.Tip(), Params().GetConsensus()) && powType != POW_TYPE_SHA256)
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Non sha256d algo requested but minotaurx not enabled");

    LOCK(cs_main);
    // ObiDoge: MinotaurX-only chain — always return MinotaurX difficulty
    return GetDifficulty(nullptr, false, POW_TYPE_MINOTAURX);
}

// ObiDoge: Hive: Get hive difficulty
...
UniValue gettxout(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of OBD addresses\n"
            "        \"address\"     (string) OBD address\n"
            "        ,...\n"
            "     ]\n"
            "  },\n"
...
UniValue getblockchaininfo(const JSONRPCRequest& request)
{
    ...
    if (IsMinotaurXEnabled(chainActive.Tip(), Params().GetConsensus()))
        obj.push_back(Pair("minotaurxdifficulty", GetDifficulty(nullptr, false, POW_TYPE_MINOTAURX)));    // ObiDoge: MinotaurX+Hive1.2
