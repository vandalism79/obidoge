...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-maxoutboundconnections=<n>", strprintf(_("Maximum number of automatic outgoing connections (default: %u)"), DEFAULT_MAX_OUTBOUND_CONNECTIONS));   // ObiDoge: Parameterisation of max outbound connections
...
// ObiDoge: Rialto option removed from help
//     strUsage += HelpMessageOpt("-rialto", strprintf(_("Support Rialto message propagration (default: %u)"), DEFAULT_RIALTO_SUPPORT));   // ObiDoge: Rialto
    ...
    // ObiDoge: Hive: Mining optimisations
    ...
    // ObiDoge: MinotaurX+Hive1.2: Allow switching of default pow algo via conf / command line, for miners that can't easily adjust their getblocktemplate calls
...
{
    const std::string URL_SOURCE_CODE = "<https://obidoge.xyz>";
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if ObiDoge is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if ObiDoge is started while in a temporary directory.\n",
    ...
    while (!fLoaded && !fRequestShutdown) {
        ...
        do {
            try {
                ...
                // ObiDoge: Rialto: MAYBEDO: Have an allow list too. The first message(s) we get from a contact, hold until we allow.
    ...
    connOptions.nMaxOutbound = std::min((int)gArgs.GetArg("-maxoutboundconnections", DEFAULT_MAX_OUTBOUND_CONNECTIONS), connOptions.nMaxConnections);    // ObiDoge: Parameterisation of max outbound connections
    ...
    // ObiDoge: Hive: Start the mining thread
#ifdef ENABLE_WALLET
